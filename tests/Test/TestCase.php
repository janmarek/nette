...
class TestCase
{
	...
	private function execute()
	{
		...
		if ($res === self::CODE_ERROR) {
			throw new TestCaseException($this->output ?: 'Fatal error', 0, $this->getName(), $this->file);

		} elseif ($res === self::CODE_FAIL) {
			throw new TestCaseException($this->output, 0, $this->getName(), $this->file);

		} elseif ($res === self::CODE_SKIP) { // skip
			throw new TestCaseException($this->output, TestCaseException::SKIPPED, $this->getName(), $this->file);
